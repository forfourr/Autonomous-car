...
from code.linedetection.hand_coded_lane_follower_230905 import HandCodedLaneFollower
...
def img_preprocess(image):
    ...
    image = image[int(height/8):int(2/3*height),:,:]
...
def test_video(video_file):
    ...
    os.makedirs(f"./{video_record}", exist_ok=True)
    os.makedirs(f"./{video_record}/{folder_name}", exist_ok=True)
    
    video_overlay_1 = cv2.VideoWriter(f"./{video_record}/{folder_name}/{folder_name}_line_detection.mp4" , video_type, 20.0, size)
    video_overlay_2 = cv2.VideoWriter(f"./{video_record}/{folder_name}/{folder_name}_end_to_end.mp4", video_type, 20.0, size)
    ...
    try :
        ...
        while cap.isOpened():
            ...
            logging.info("desired=%3d, model=%3d, diff=%3d" %
                          ...
                          diff))
            video_overlay_1.write(combo_image1)
            video_overlay_2.write(combo_image2)
        ...
        cap.release()
        video_overlay_1.release()
        video_overlay_2.release()
        cv2.destroyAllWindows()

...
def detect_edges(frame):
    ...
    cropped_image = gray[340:480,0:640]
...
def compute_steering_angle(frame, lane_lines):
    ...
    if len(lane_lines) == 1:
        ...
        # angle creation via right lane
        if x1 > x2:
            x_offset = x2 - x1
            y_offset = y1 - y2 + 40

        # angle creation via left lane
        else:
            x_offset = x1 - x2
            y_offset = y2 - y1 
...
def stabilize_steering_angle(curr_steering_angle, new_steering_angle, num_of_lane_lines, max_angle_deviation_two_lines=3, max_angle_deviation_one_lane=30):
    """

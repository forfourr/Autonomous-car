...
def detect_lane(frame):
    ...
    # edges = detect_edges_gray(frame)
    # show_image('edges_gray', edges)

    # cropped_edges = region_of_interest(edges)
    # show_image('edges cropped', cropped_edges)

    cropped_edges = detect_edges_gray(frame)
    show_image('edges cropped', cropped_edges)
...
def detect_edges_gray(frame):
    ...
    cropped_image = gray[330:480, 0:640]
...
def compute_steering_angle(frame, lane_lines):
    ...
    """
    print('num of lanes', len(lane_lines))
    if len(lane_lines) == 0:
    ...
    if lines is not None:
        for line in lines:
            for x1, y1, x2, y2 in line:
                cv2.line(line_image, (x1, y1+330), (x2, y2+330), line_color, line_width)
...
if __name__ == '__main__':
    ...
    test_video('./DeepPiCar/driver/data/task_last3.mp4')
